...
void main()
{
    ...
    vec3 adjustedViewPos = vec3(viewPos.x,0.0,viewPos.z - 2.0);
    vec3 viewDir = normalize(adjustedViewPos - FragPos);
    vec3 reflectDir = reflect(-lightDir, norm);

    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32);
    vec3 specular = specularStrength * spec * vec3(1.0, 1.0, 1.0); //removes light color here
